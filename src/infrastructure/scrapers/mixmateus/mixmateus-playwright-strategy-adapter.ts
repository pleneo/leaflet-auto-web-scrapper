...
export interface MixMateusPlaywrightExtractionPort {
  extract(input: ExtractMixMateusLeafletsInput): Promise<MixMateusLeafletExtractionResult>;
}

export interface MixMateusPlaywrightStoragePort {
  store(input: StoreSharedPdfLeafletExtractionInput): Promise<StoredSharedPdfLeafletExtraction>;
}

export interface MixMateusPlaywrightStrategyAdapterConfig {
  readonly extractionInput: Omit<ExtractMixMateusLeafletsInput, 'visualDataset'>;
  readonly outputRootDirectory: string;
  readonly visualDatasetRootDirectory: string;
  readonly visualDatasetSplit: DatasetSplit;
}

export interface MixMateusPlaywrightStrategyAdapterDependencies {
  readonly extractionService: MixMateusPlaywrightExtractionPort;
  readonly storage: MixMateusPlaywrightStoragePort;
  readonly countVisualDatasetSamples: (rootDirectory: string, runId: string) => Promise<number>;
}

export class MixMateusPlaywrightStrategyAdapter implements PlaywrightExtractionStrategy {
  readonly supermarketId: SupermarketId = 'mixmateus';

  private readonly config: MixMateusPlaywrightStrategyAdapterConfig;

  private readonly extractionService: MixMateusPlaywrightExtractionPort;

  private readonly storage: MixMateusPlaywrightStoragePort;
  ...
  constructor(
    config: MixMateusPlaywrightStrategyAdapterConfig,
    dependencies: MixMateusPlaywrightStrategyAdapterDependencies,
  ) {
    this.config = config;
...
function createStorageInput(
  config: MixMateusPlaywrightStrategyAdapterConfig,
  result: MixMateusLeafletExtractionResult,
): StoreSharedPdfLeafletExtractionInput {
  return {
