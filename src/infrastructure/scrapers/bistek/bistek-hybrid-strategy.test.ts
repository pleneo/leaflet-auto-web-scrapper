...
describe('BistekHybridStrategy', () => {
  it('uses API first when visual dataset capture is disabled and skips Playwright on API success', async () => {
    ...
    expect(loggerInfo).toHaveBeenCalledWith(
      'Bistek store extracted successfully via API.',
      expect.objectContaining({ storeId: '1', storeName: 'Store 1' }),
    );
  });
  ...
  it('falls back to Playwright and merges outputs when API has failing store units', async () => {
    const apiStrategy = new FakeStrategy(
      'api',
