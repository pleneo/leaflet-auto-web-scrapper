  ...
  async execute(input: ExtractionStrategyInput): Promise<ExtractionStrategyOutput> {
    ...
    for (const unit of apiOutput.units) {
      if (unit.status === 'succeeded' || unit.status === 'empty') {
        input.logger.info('Coop store extracted successfully via API.', {
          runId: input.runId,
    ...
    const homeOutput = await this.homePlaywrightStrategy.execute(input);

    return apiOutput.units.length > 0 ? mergeStrategyOutputs(apiOutput, homeOutput) : homeOutput;
  }
}
